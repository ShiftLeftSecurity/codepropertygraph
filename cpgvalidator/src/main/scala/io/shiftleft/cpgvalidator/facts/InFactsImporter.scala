...
class InFactsImporter extends FactsImporter {
  ...
  override def loadFacts: List[InFact] =
    List(
      ...
      NodeTypes.TYPE_REF has 1 incoming EdgeTypes.AST from SuperTypes.Expression or NodeTypes.CONTROL_STRUCTURE,
      NodeTypes.TYPE_REF has 0 to N incoming EdgeTypes.CFG from SuperTypes.Expression or NodeTypes.METHOD,
      NodeTypes.TYPE_REF has 0 to 1 incoming EdgeTypes.RECEIVER from NodeTypes.CALL,
      NodeTypes.TYPE_REF has 0 to 1 incoming EdgeTypes.ARGUMENT from NodeTypes.CALL or NodeTypes.RETURN,
