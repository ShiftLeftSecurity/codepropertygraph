...
class HighlightedSourceTest extends WordSpec with Matchers {
  ...
  "fansi encoding fix" must {
    "handle different ansi encoding termination" in {
      ...
      val fixedForFansi = HighlightedSource(IntGreenForeground).fixedForFansi
    ...
    "handle different single-digit encodings" in {
      ...
      val fixedForFansi = HighlightedSource(FBold).fixedForFansi
    ...
    "handle multi-encoded parts" in {
      ...
      val fixedForFansi = HighlightedSource(IfBlueBold).fixedForFansi
