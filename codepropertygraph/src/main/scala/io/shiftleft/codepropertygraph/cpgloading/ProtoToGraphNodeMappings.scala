 ...
 * 
 * Context: we need to run two passes: 1) add nodes and 2) set node properties and add edges
 * (this is due to flatgraph-specific implementation details)
 *
 * Because of that, we need to remember the mapping from proto node id to gnode. Typically that's just a plain
 * mapping, but there's one special case for TYPE nodes: some (parallel) frontends create duplicate TYPE nodes
 * which we need to deduplicate...
 */
class ProtoToGraphNodeMappings {
  ...
  def addAll(other: ProtoToGraphNodeMappings): Unit = {
    ...
    assert(intersection1.isEmpty, s"unexpected duplicate entries in protoNodeIdToGNode mappings. protoNodeIds: $intersection1")
    assert(intersection2.isEmpty, s"unexpected duplicate entries in typeFullNameToGNode mappings. FullNames: $intersection2")
  ...
  def add(protoNode: CpgStruct.Node, node: DNode): Unit = {
    ...
    typeFullNameMaybe(protoNode).foreach(typeFullName =>
      typeFullNameToGNode += typeFullName -> node
    )
  }
  ...
  /** This will fail hard if the DiffGraph hasn't been applied yet, which is the assumption for it's use case.
   * In other words, we specifically don't want to invoke `find(protoNode).flatMap(_.storedRef)` here
   * */
   ...
   * In other words, we specifically don't want to invoke `find(protoNode).flatMap(_.storedRef)` here
   *
   * Note that this doesn't (and cannot) check for duplicate nodes with the same TYPE.FULL_NAME, because we only
   * have the protoNodeId and not the node itself.
   * */
  ...
  private def typeFullNameMaybe(protoNode: CpgStruct.Node): Option[String] = {
    if (protoNode.getType == CpgStruct.Node.NodeType.TYPE) {
      protoNode.getPropertyList.asScala.collectFirst {
        case prop if prop.getName == io.shiftleft.proto.cpg.Cpg.NodePropertyName.FULL_NAME => prop.getValue.getStringValue
      }
    } else {
      None
    }
