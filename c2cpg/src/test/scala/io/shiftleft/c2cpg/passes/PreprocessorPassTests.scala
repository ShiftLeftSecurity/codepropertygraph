  ...
  "PreprocessorPass" should {
    ...
    "find all ifdefs and ifs with predefined symbols" in {
      File.usingTemporaryDirectory("preprocessorPassTests") { dir =>
        ...
        val parserConfig =
          ParserConfig(Set.empty, Set.empty, Map("SYMBOL" -> "true"), logProblems = false, logPreprocessor = false)
        val stmts = new PreprocessorPass(List(filePath), parserConfig).run().toList
