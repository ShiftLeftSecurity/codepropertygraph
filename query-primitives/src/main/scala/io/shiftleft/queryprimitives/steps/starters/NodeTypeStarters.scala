...
object NodeTypeStarters {
  ...
  /* TODO MP: generate in DomainClassCreator - note there is special handling in cpg-internal currently */
  val storedNodeMarshaller: Marshallable[nodes.StoredNode] = new Marshallable[nodes.StoredNode] {
    override def toCC(element: Element): nodes.StoredNode =
      element.label match {
        case "ARRAY_INITIALIZER" => implicitly[Marshallable[nodes.ArrayInitializer]].toCC(element)
        case "BLOCK" => implicitly[Marshallable[nodes.Block]].toCC(element)
        case "CALL" => implicitly[Marshallable[nodes.Call]].toCC(element)
        case "CLOSURE_BINDING" => implicitly[Marshallable[nodes.ClosureBinding]].toCC(element)
        case "FILE" => implicitly[Marshallable[nodes.File]].toCC(element)
        ...
        case "META_DATA" => implicitly[Marshallable[nodes.MetaData]].toCC(element)
        case "METHOD" => implicitly[Marshallable[nodes.Method]].toCC(element)
        case "METHOD_INST" => implicitly[Marshallable[nodes.MethodInst]].toCC(element)
        ...
        case "METHOD_RETURN" => implicitly[Marshallable[nodes.MethodReturn]].toCC(element)
        case "MODIFIER" => implicitly[Marshallable[nodes.Modifier]].toCC(element)
        case "NAMESPACE" => implicitly[Marshallable[nodes.Namespace]].toCC(element)
        case "NAMESPACE_BLOCK" => implicitly[Marshallable[nodes.NamespaceBlock]].toCC(element)
        case "RETURN" => implicitly[Marshallable[nodes.Return]].toCC(element)
        case "TAG" => implicitly[Marshallable[nodes.Tag]].toCC(element)
        case "TYPE" => implicitly[Marshallable[nodes.Type]].toCC(element)
        case "TYPE_ARGUMENT" => implicitly[Marshallable[nodes.TypeArgument]].toCC(element)
        case "TYPE_DECL" => implicitly[Marshallable[nodes.TypeDecl]].toCC(element)
        case "TYPE_PARAMETER" => implicitly[Marshallable[nodes.TypeParameter]].toCC(element)
