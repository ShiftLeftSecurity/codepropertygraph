...
class Method(override val raw: GremlinScala[nodes.Method])
  ...
  /**
    ...
    * Traverse to direct and transitive callers of the method.
    */
  private[semanticcpg] def calledByIncludingSink(sourceTrav: Method, resolve: Boolean = true)(
      implicit callResolver: ICallResolver): Method = {
    val sourceMethods = sourceTrav.raw.toSet
    val sinkMethods = raw.dedup.toList()

    if (sourceMethods.isEmpty || sinkMethods.isEmpty) {
      new Method(graph.V(-1).asInstanceOf[GremlinScala[nodes.Method]])
    } else {
