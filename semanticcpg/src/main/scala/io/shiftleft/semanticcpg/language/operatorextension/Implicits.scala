...
trait Implicits {
  ...
  implicit def toArrayAccessTrav(steps: Traversal[opnodes.ArrayAccess]): ArrayAccess = new ArrayAccess(steps)
  implicit def toAssignmentExt(assignment: opnodes.Assignment): AssignmentMethods = new AssignmentMethods(assignment)
  implicit def toAssignmentTrav(steps: Traversal[opnodes.Assignment]): Assignment = new Assignment(steps)
  implicit def toTargetExt(call: nodes.Expression): TargetMethods = new TargetMethods(call)
  implicit def toTargetTrav(steps: Traversal[nodes.Expression]): Target = new Target(steps)
