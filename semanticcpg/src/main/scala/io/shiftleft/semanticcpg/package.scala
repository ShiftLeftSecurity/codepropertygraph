...
/**
  ...
  *
  * This is the API reference for the CPG query language, a language to mine code for
  * defects and vulnerabilities both interactively on a code analysis shell (REPL), or
  * using non-interactive scripts.
  *
  * Queries written in the CPG query language express graph traversals
  * (see [[https://en.wikipedia.org/wiki/Graph_traversal]]). Similar to the standard
  * graph traversal language "Gremlin" (see [[https://en.wikipedia.org/wiki/Gremlin_(programming_language)]]))
  * these traversals are formulated as sequences of primitive language elements referred to as "steps".
  * You can think of a step as a small program, similar to a unix shell utility, however, instead
  * of processing lines one by one, the step processes nodes of the graph.
  *
  ...
  * {{{ cpg.local }}}
  *
  * will start at all local variables. The complete list of starting points can
  * be found at {{{ io.shiftleft.codepropertygraph.Cpg }}}
  *
  * == Lazy evaluation ==
  ...
  *
  * == Properties ==
  * Nodes have "properties", key-value pairs where keys are strings and values are primitive
  * data types such as strings, integers, or Booleans. Properties of nodes can be selected based
  * on their key, e.g.,
  *
  * {{{ cpg.method.name }}}
  *
  ...
  * This prints all typeDecl names as it traverses the graph and increments `i` for each one.
  * {{{
  *   var i = 0
  ...
  * derived the correct return type as a tuple of the labelled steps, in this case with two elements.
  *
  * {{{
