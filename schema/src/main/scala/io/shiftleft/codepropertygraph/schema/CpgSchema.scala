...
class CpgSchema(builder: SchemaBuilder) {
  ...
  val tagsAndLocation = TagsAndLocation(builder, base, fs, callGraph)
  ...
  val hidden          = Hidden(builder, base, method, typeSchema, ast, cfg, fs, callGraph, pdg, tagsAndLocation)
