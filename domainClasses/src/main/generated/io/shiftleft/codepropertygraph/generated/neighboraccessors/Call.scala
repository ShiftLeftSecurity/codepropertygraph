...
def controlStructureViaAstIn: nodes.ControlStructure = {
  try { astIn.collectAll[nodes.ControlStructure].next() } catch {
    case e: java.util.NoSuchElementException =>
      throw new flatgraph.SchemaViolationException("IN edge with label AST to an adjacent CONTROL_STRUCTURE is mandatory, but not defined for this CALL node with seq=" + node.seq, e)
  }
}
 ...
 * Traverse to MEMBER via REF OUT edge. */
...
/** 
 * Traverse to CONTROL_STRUCTURE via RECEIVER OUT edge. */
def controlStructureViaReceiverOut: Iterator[nodes.ControlStructure] = traversal.flatMap(_.controlStructureViaReceiverOut)


/** 
 * Traverse to FIELD_IDENTIFIER via ARGUMENT OUT edge. */
def fieldIdentifierViaArgumentOut: Iterator[nodes.FieldIdentifier] = traversal.flatMap(_.fieldIdentifierViaArgumentOut)
