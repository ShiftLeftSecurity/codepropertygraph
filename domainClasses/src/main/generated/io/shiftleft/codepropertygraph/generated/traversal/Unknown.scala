...
class UnknownTraversalExtGen[NodeType <: Unknown](val traversal: Iterator[NodeType]) extends AnyVal {
  ...
  def containedRef(pattern: String): Iterator[NodeType] = {
    if (!Misc.isRegex(pattern)) {
      ...
      overflowdb.traversal.filter.StringPropertyFilter.regexp(traversal)(_.containedRef, pattern)
    }
  }
  ...
  def containedRef(patterns: String*): Iterator[NodeType] =
    overflowdb.traversal.filter.StringPropertyFilter.regexpMultiple(traversal)(_.containedRef, patterns)
  ...
  def containedRefExact(value: String): Iterator[NodeType] = {
    val fastResult = traversal match {
      case init: overflowdb.traversal.InitialTraversal[NodeType] =>
        init.getByIndex("CONTAINED_REF", value).getOrElse(null)
      case _ => null
    }
    if (fastResult != null) fastResult
    else traversal.filter { node => node.containedRef == value }
  }
  ...
  def containedRefExact(values: String*): Iterator[NodeType] = {
    if (values.size == 1)
      containedRefExact(values.head)
    else
      overflowdb.traversal.filter.StringPropertyFilter
        .exactMultiple[NodeType, String](traversal, node => Some(node.containedRef), values, "CONTAINED_REF")
  }
  ...
  def containedRefNot(pattern: String): Iterator[NodeType] = {
    if (!Misc.isRegex(pattern)) {
      ...
      overflowdb.traversal.filter.StringPropertyFilter.regexpNot(traversal)(_.containedRef, pattern)
    }
  }
  ...
  def containedRefNot(patterns: String*): Iterator[NodeType] = {
    overflowdb.traversal.filter.StringPropertyFilter.regexpNotMultiple(traversal)(_.containedRef, patterns)
  }
