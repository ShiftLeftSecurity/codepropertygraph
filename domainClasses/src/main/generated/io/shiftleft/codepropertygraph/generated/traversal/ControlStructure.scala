...
class ControlStructureTraversalExtGen[NodeType <: ControlStructure](val traversal: Iterator[NodeType]) extends AnyVal {
  ...
  def controlStructureType(pattern: String): Iterator[NodeType] = {
    if (!Misc.isRegex(pattern)) {
      ...
      overflowdb.traversal.filter.StringPropertyFilter.regexp(traversal)(_.controlStructureType, pattern)
    }
  }
  ...
  def controlStructureType(patterns: String*): Iterator[NodeType] =
    overflowdb.traversal.filter.StringPropertyFilter.regexpMultiple(traversal)(_.controlStructureType, patterns)
  ...
  def controlStructureTypeExact(value: String): Iterator[NodeType] = {
    val fastResult = traversal match {
      case init: overflowdb.traversal.InitialTraversal[NodeType] =>
        init.getByIndex("CONTROL_STRUCTURE_TYPE", value).getOrElse(null)
      case _ => null
    }
    if (fastResult != null) fastResult
    else traversal.filter { node => node.controlStructureType == value }
  }
  ...
  def controlStructureTypeNot(pattern: String): Iterator[NodeType] = {
    if (!Misc.isRegex(pattern)) {
      ...
      overflowdb.traversal.filter.StringPropertyFilter.regexpNot(traversal)(_.controlStructureType, pattern)
    }
  }
  ...
  def controlStructureTypeNot(patterns: String*): Iterator[NodeType] = {
    overflowdb.traversal.filter.StringPropertyFilter.regexpNotMultiple(traversal)(_.controlStructureType, patterns)
  }
  ...
  def parserTypeName(pattern: String): Iterator[NodeType] = {
    if (!Misc.isRegex(pattern)) {
      ...
      overflowdb.traversal.filter.StringPropertyFilter.regexp(traversal)(_.parserTypeName, pattern)
    }
  }
  ...
  def parserTypeName(patterns: String*): Iterator[NodeType] =
    overflowdb.traversal.filter.StringPropertyFilter.regexpMultiple(traversal)(_.parserTypeName, patterns)
  ...
  def parserTypeNameExact(value: String): Iterator[NodeType] = {
    val fastResult = traversal match {
      case init: overflowdb.traversal.InitialTraversal[NodeType] =>
        init.getByIndex("PARSER_TYPE_NAME", value).getOrElse(null)
      case _ => null
    }
    if (fastResult != null) fastResult
    else traversal.filter { node => node.parserTypeName == value }
  }
  ...
  def parserTypeNameExact(values: String*): Iterator[NodeType] = {
    if (values.size == 1)
      parserTypeNameExact(values.head)
    else
      overflowdb.traversal.filter.StringPropertyFilter
        .exactMultiple[NodeType, String](traversal, node => Some(node.parserTypeName), values, "PARSER_TYPE_NAME")
  }
  ...
  def parserTypeNameNot(pattern: String): Iterator[NodeType] = {
    if (!Misc.isRegex(pattern)) {
      ...
      overflowdb.traversal.filter.StringPropertyFilter.regexpNot(traversal)(_.parserTypeName, pattern)
    }
  }
