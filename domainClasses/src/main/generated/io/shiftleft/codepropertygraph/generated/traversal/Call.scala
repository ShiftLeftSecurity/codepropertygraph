...
class CallTraversalExtGen[NodeType <: Call](val traversal: Iterator[NodeType]) extends AnyVal {
  ...
  def referencedMember: Iterator[Member] =
    traversal.flatMap(_.referencedMember)
  ...
  def dispatchType(pattern: String): Iterator[NodeType] = {
    if (!Misc.isRegex(pattern)) {
      ...
      overflowdb.traversal.filter.StringPropertyFilter.regexp(traversal)(_.dispatchType, pattern)
    }
  }
  ...
  def dispatchType(patterns: String*): Iterator[NodeType] =
    overflowdb.traversal.filter.StringPropertyFilter.regexpMultiple(traversal)(_.dispatchType, patterns)
  ...
  def dispatchTypeExact(value: String): Iterator[NodeType] = {
    val fastResult = traversal match {
      case init: overflowdb.traversal.InitialTraversal[NodeType] =>
        init.getByIndex("DISPATCH_TYPE", value).getOrElse(null)
      case _ => null
    }
    if (fastResult != null) fastResult
    else traversal.filter { node => node.dispatchType == value }
  }
  ...
  def dispatchTypeExact(values: String*): Iterator[NodeType] = {
    if (values.size == 1)
      dispatchTypeExact(values.head)
    else
      overflowdb.traversal.filter.StringPropertyFilter
        .exactMultiple[NodeType, String](traversal, node => Some(node.dispatchType), values, "DISPATCH_TYPE")
  }
  ...
  def dispatchTypeNot(pattern: String): Iterator[NodeType] = {
    if (!Misc.isRegex(pattern)) {
      ...
      overflowdb.traversal.filter.StringPropertyFilter.regexpNot(traversal)(_.dispatchType, pattern)
    }
  }
  ...
  def dispatchTypeNot(patterns: String*): Iterator[NodeType] = {
    overflowdb.traversal.filter.StringPropertyFilter.regexpNotMultiple(traversal)(_.dispatchType, patterns)
  }
