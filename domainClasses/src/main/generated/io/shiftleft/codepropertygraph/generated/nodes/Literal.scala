...
class Literal(graph_4762: Graph, id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */ )
  ...
  /** Traverse to ARRAY_INITIALIZER via AST IN edge.
    */
  def arrayInitializerViaAstIn: overflowdb.traversal.Traversal[ArrayInitializer] = get().arrayInitializerViaAstIn

  @deprecated("please use `arrayInitializerViaAstIn`", "June 2024")
  def _arrayInitializerViaAstIn = arrayInitializerViaAstIn
...
class LiteralDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode with Expression with LiteralBase {
  ...
  private var _argumentIndex: Integer                      = Literal.PropertyDefaults.ArgumentIndex
  ...
  private var _order: Integer                              = Literal.PropertyDefaults.Order
  ...
  def arrayInitializerViaAstIn: overflowdb.traversal.Traversal[ArrayInitializer] = astIn.collectAll[ArrayInitializer]
  @deprecated("please use `blockViaAstIn`", "June 2024")
  def _blockViaAstIn = blockViaAstIn
  ...
  override def fromNewNode(newNode: NewNode, mapping: NewNode => StoredNode): Unit = {
    ...
    this._argumentName = newNode.asInstanceOf[NewLiteral].argumentName match {
      case None => null; case Some(value) => value
    }
    this._code = newNode.asInstanceOf[NewLiteral].code
    this._columnNumber = newNode.asInstanceOf[NewLiteral].columnNumber match {
      case None => null; case Some(value) => value
    }
    ...
    this._lineNumber = newNode.asInstanceOf[NewLiteral].lineNumber match {
      case None => null; case Some(value) => value
    }
