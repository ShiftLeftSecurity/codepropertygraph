...
def arrayInitializerViaAstIn: Iterator[nodes.ArrayInitializer] = astIn.collectAll[nodes.ArrayInitializer]
...
def controlStructureViaAstIn: nodes.ControlStructure = {
  try { astIn.collectAll[nodes.ControlStructure].next() } catch {
    case e: java.util.NoSuchElementException =>
      throw new flatgraph.SchemaViolationException("IN edge with label AST to an adjacent CONTROL_STRUCTURE is mandatory, but not defined for this LITERAL node with seq=" + node.seq, e)
  }
}
...
/** 
 * Traverse to ARRAY_INITIALIZER via AST IN edge. */
def arrayInitializerViaAstIn: Iterator[nodes.ArrayInitializer] = traversal.flatMap(_.arrayInitializerViaAstIn)
