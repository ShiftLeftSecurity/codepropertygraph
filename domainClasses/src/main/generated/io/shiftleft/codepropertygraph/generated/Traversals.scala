...
object Accessors {
  ...
  final class Traversal_Property_ALIAS_TYPE_FULL_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasAliasTypeFullNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to aliasTypeFullName property */
...
def aliasTypeFullNameNot(pattern: String): Iterator[NodeType] = {
  if(!Misc.isRegex(pattern)){
    ...
    val matcher = java.util.regex.Pattern.compile(pattern).matcher("")
    traversal.filterNot{ item =>  val tmp = item.aliasTypeFullName; tmp.isDefined && matcher.reset(tmp.get).matches}
  }
}
...
def aliasTypeFullNameNot(patterns: String*): Iterator[NodeType] = {
  val matchers = patterns.map{java.util.regex.Pattern.compile(_).matcher("")}
  traversal.filterNot{item => val tmp = item.aliasTypeFullName; tmp.isDefined && matchers.exists{_.reset(tmp.get).matches}}
 }

}
final class Traversal_Property_ARGUMENT_INDEX[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasArgumentIndexEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to argumentIndex property */
...
final class Traversal_Property_ARGUMENT_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasArgumentNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to argumentName property */
...
def argumentNameNot(pattern: String): Iterator[NodeType] = {
  if(!Misc.isRegex(pattern)){
    ...
    val matcher = java.util.regex.Pattern.compile(pattern).matcher("")
    traversal.filterNot{ item =>  val tmp = item.argumentName; tmp.isDefined && matcher.reset(tmp.get).matches}
  }
}
...
def argumentNameNot(patterns: String*): Iterator[NodeType] = {
  val matchers = patterns.map{java.util.regex.Pattern.compile(_).matcher("")}
  traversal.filterNot{item => val tmp = item.argumentName; tmp.isDefined && matchers.exists{_.reset(tmp.get).matches}}
 }

}
final class Traversal_Property_AST_PARENT_FULL_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasAstParentFullNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to astParentFullName property */
...
def astParentFullNameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  3, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_AST_PARENT_TYPE[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasAstParentTypeEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to astParentType property */
...
final class Traversal_Property_CANONICAL_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasCanonicalNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to canonicalName property */
...
final class Traversal_Property_CLASS_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasClassNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to className property */
...
def classNameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  6, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_CLASS_SHORT_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasClassShortNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to classShortName property */
...
def classShortNameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  7, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_CLOSURE_BINDING_ID[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasClosureBindingIdEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to closureBindingId property */
...
def closureBindingIdNot(pattern: String): Iterator[NodeType] = {
  if(!Misc.isRegex(pattern)){
    ...
    val matcher = java.util.regex.Pattern.compile(pattern).matcher("")
    traversal.filterNot{ item =>  val tmp = item.closureBindingId; tmp.isDefined && matcher.reset(tmp.get).matches}
  }
}
...
def closureBindingIdNot(patterns: String*): Iterator[NodeType] = {
  val matchers = patterns.map{java.util.regex.Pattern.compile(_).matcher("")}
  traversal.filterNot{item => val tmp = item.closureBindingId; tmp.isDefined && matchers.exists{_.reset(tmp.get).matches}}
 }

}
final class Traversal_Property_CLOSURE_ORIGINAL_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasClosureOriginalNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to closureOriginalName property */
...
def closureOriginalNameNot(pattern: String): Iterator[NodeType] = {
  if(!Misc.isRegex(pattern)){
    ...
    val matcher = java.util.regex.Pattern.compile(pattern).matcher("")
    traversal.filterNot{ item =>  val tmp = item.closureOriginalName; tmp.isDefined && matcher.reset(tmp.get).matches}
  }
}
...
def closureOriginalNameNot(patterns: String*): Iterator[NodeType] = {
  val matchers = patterns.map{java.util.regex.Pattern.compile(_).matcher("")}
  traversal.filterNot{item => val tmp = item.closureOriginalName; tmp.isDefined && matchers.exists{_.reset(tmp.get).matches}}
 }

}
final class Traversal_Property_CODE[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasCodeEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to code property */
...
def codeExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  10, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_COLUMN_NUMBER[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasColumnNumberEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to columnNumber property */
...
final class Traversal_Property_COLUMN_NUMBER_END[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasColumnNumberEndEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to columnNumberEnd property */
...
final class Traversal_Property_CONTAINED_REF[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasContainedRefEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to containedRef property */
...
def containedRefExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  13, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_CONTENT[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasContentEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to content property */
...
def contentExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  14, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_CONTROL_STRUCTURE_TYPE[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasControlStructureTypeEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to controlStructureType property */
...
def controlStructureTypeExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  15, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_DEPENDENCY_GROUP_ID[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasDependencyGroupIdEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to dependencyGroupId property */
...
def dependencyGroupIdExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  16, value).asInstanceOf[Iterator[NodeType]]
...
def dependencyGroupIdNot(pattern: String): Iterator[NodeType] = {
  if(!Misc.isRegex(pattern)){
    ...
    val matcher = java.util.regex.Pattern.compile(pattern).matcher("")
    traversal.filterNot{ item =>  val tmp = item.dependencyGroupId; tmp.isDefined && matcher.reset(tmp.get).matches}
  }
}
...
def dependencyGroupIdNot(patterns: String*): Iterator[NodeType] = {
  val matchers = patterns.map{java.util.regex.Pattern.compile(_).matcher("")}
  traversal.filterNot{item => val tmp = item.dependencyGroupId; tmp.isDefined && matchers.exists{_.reset(tmp.get).matches}}
 }

}
final class Traversal_Property_DISPATCH_TYPE[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasDispatchTypeEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to dispatchType property */
...
def dispatchTypeExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  17, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_DYNAMIC_TYPE_HINT_FULL_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasDynamicTypeHintFullNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to dynamicTypeHintFullName property */
def dynamicTypeHintFullName: Iterator[String] =
  traversal.flatMap(_.dynamicTypeHintFullName)


}
final class Traversal_Property_EVALUATION_STRATEGY[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasEvaluationStrategyEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to evaluationStrategy property */
...
def evaluationStrategyExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  19, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_EXPLICIT_AS[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasExplicitAsEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to explicitAs property */
...
final class Traversal_Property_FILENAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasFilenameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to filename property */
...
def filenameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  21, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_FULL_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasFullNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to fullName property */
...
def fullNameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  22, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_HASH[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasHashEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to hash property */
...
def hashExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  23, value).asInstanceOf[Iterator[NodeType]]
...
def hashNot(pattern: String): Iterator[NodeType] = {
  if(!Misc.isRegex(pattern)){
    ...
    val matcher = java.util.regex.Pattern.compile(pattern).matcher("")
    traversal.filterNot{ item =>  val tmp = item.hash; tmp.isDefined && matcher.reset(tmp.get).matches}
  }
}
...
def hashNot(patterns: String*): Iterator[NodeType] = {
  val matchers = patterns.map{java.util.regex.Pattern.compile(_).matcher("")}
  traversal.filterNot{item => val tmp = item.hash; tmp.isDefined && matchers.exists{_.reset(tmp.get).matches}}
 }

}
final class Traversal_Property_IMPORTED_AS[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasImportedAsEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to importedAs property */
...
def importedAsExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  24, value).asInstanceOf[Iterator[NodeType]]
...
def importedAsNot(pattern: String): Iterator[NodeType] = {
  if(!Misc.isRegex(pattern)){
    ...
    val matcher = java.util.regex.Pattern.compile(pattern).matcher("")
    traversal.filterNot{ item =>  val tmp = item.importedAs; tmp.isDefined && matcher.reset(tmp.get).matches}
  }
}
...
def importedAsNot(patterns: String*): Iterator[NodeType] = {
  val matchers = patterns.map{java.util.regex.Pattern.compile(_).matcher("")}
  traversal.filterNot{item => val tmp = item.importedAs; tmp.isDefined && matchers.exists{_.reset(tmp.get).matches}}
 }

}
final class Traversal_Property_IMPORTED_ENTITY[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasImportedEntityEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to importedEntity property */
...
def importedEntityExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  25, value).asInstanceOf[Iterator[NodeType]]
...
def importedEntityNot(pattern: String): Iterator[NodeType] = {
  if(!Misc.isRegex(pattern)){
    ...
    val matcher = java.util.regex.Pattern.compile(pattern).matcher("")
    traversal.filterNot{ item =>  val tmp = item.importedEntity; tmp.isDefined && matcher.reset(tmp.get).matches}
  }
}
...
def importedEntityNot(patterns: String*): Iterator[NodeType] = {
  val matchers = patterns.map{java.util.regex.Pattern.compile(_).matcher("")}
  traversal.filterNot{item => val tmp = item.importedEntity; tmp.isDefined && matchers.exists{_.reset(tmp.get).matches}}
 }

}
final class Traversal_Property_INDEX[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasIndexEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to index property */
...
final class Traversal_Property_INHERITS_FROM_TYPE_FULL_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasInheritsFromTypeFullNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to inheritsFromTypeFullName property */
def inheritsFromTypeFullName: Iterator[String] =
  traversal.flatMap(_.inheritsFromTypeFullName)


}
final class Traversal_Property_IS_EXPLICIT[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasIsExplicitEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to isExplicit property */
...
final class Traversal_Property_IS_EXTERNAL[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasIsExternalEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to isExternal property */
...
final class Traversal_Property_IS_VARIADIC[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasIsVariadicEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to isVariadic property */
...
final class Traversal_Property_IS_WILDCARD[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasIsWildcardEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to isWildcard property */
...
final class Traversal_Property_KEY[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasKeyEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to key property */
...
def keyExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  32, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_LANGUAGE[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasLanguageEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to language property */
...
def languageExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  33, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_LINE_NUMBER[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasLineNumberEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to lineNumber property */
...
final class Traversal_Property_LINE_NUMBER_END[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasLineNumberEndEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to lineNumberEnd property */
...
final class Traversal_Property_METHOD_FULL_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasMethodFullNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to methodFullName property */
...
def methodFullNameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  36, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_METHOD_SHORT_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasMethodShortNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to methodShortName property */
...
def methodShortNameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  37, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_MODIFIER_TYPE[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasModifierTypeEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to modifierType property */
...
def modifierTypeExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  38, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to name property */
...
def nameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  39, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_NODE_LABEL[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasNodeLabelEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to nodeLabel property */
...
def nodeLabelExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  40, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_OFFSET[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasOffsetEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to offset property */
...
def offset(value: Int): Iterator[NodeType] =
  traversal.filter{node => val tmp = node.offset; tmp.isDefined && tmp.get == value}
...
def offset(values: Int*): Iterator[NodeType] = {
  val vset = values.toSet
  traversal.filter{node => val tmp = node.offset; tmp.isDefined && vset.contains(tmp.get)}
}
...
final class Traversal_Property_OFFSET_END[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasOffsetEndEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to offsetEnd property */
...
def offsetEnd(value: Int): Iterator[NodeType] =
  traversal.filter{node => val tmp = node.offsetEnd; tmp.isDefined && tmp.get == value}
...
def offsetEnd(values: Int*): Iterator[NodeType] = {
  val vset = values.toSet
  traversal.filter{node => val tmp = node.offsetEnd; tmp.isDefined && vset.contains(tmp.get)}
}
...
final class Traversal_Property_ORDER[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasOrderEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to order property */
...
final class Traversal_Property_OVERLAYS[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasOverlaysEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to overlays property */
def overlays: Iterator[String] =
  traversal.flatMap(_.overlays)


}
final class Traversal_Property_PACKAGE_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasPackageNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to packageName property */
...
def packageNameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  45, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_PARSER_TYPE_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasParserTypeNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to parserTypeName property */
...
def parserTypeNameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  46, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_POSSIBLE_TYPES[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasPossibleTypesEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to possibleTypes property */
def possibleTypes: Iterator[String] =
  traversal.flatMap(_.possibleTypes)


}
final class Traversal_Property_ROOT[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasRootEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to root property */
...
def rootExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  48, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_SIGNATURE[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasSignatureEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to signature property */
...
def signatureExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  49, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_SYMBOL[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasSymbolEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to symbol property */
...
def symbolExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  50, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_TYPE_DECL_FULL_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasTypeDeclFullNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to typeDeclFullName property */
...
def typeDeclFullNameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  51, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_TYPE_FULL_NAME[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasTypeFullNameEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to typeFullName property */
...
def typeFullNameExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  52, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_VALUE[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasValueEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to value property */
...
def valueExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  53, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_Property_VERSION[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasVersionEMT]](val traversal: Iterator[NodeType]) extends AnyVal {/** Traverse to version property */
...
def versionExact(value: String): Iterator[NodeType] = traversal match {
    case init: flatgraph.misc.InitNodeIterator[flatgraph.GNode @unchecked] if init.isVirgin && init.hasNext =>
      val someNode = init.next
      flatgraph.Accessors.getWithInverseIndex(someNode.graph, someNode.nodeKind,  54, value).asInstanceOf[Iterator[NodeType]]
...
final class Traversal_FileBase[NodeType <: nodes.FileBase](val traversal: Iterator[NodeType]) extends AnyVal { /** Traverse to content property */
def content: Iterator[String] =
  traversal.map(_.content)
...
final class Traversal_MemberBase[NodeType <: nodes.MemberBase](val traversal: Iterator[NodeType]) extends AnyVal { /** Traverse to astParentFullName property */
def astParentFullName: Iterator[String] =
  traversal.map(_.astParentFullName)
...
final class Traversal_MethodParameterInBase[NodeType <: nodes.MethodParameterInBase](val traversal: Iterator[NodeType]) extends AnyVal { /** Traverse to closureBindingId property */
def closureBindingId: Iterator[String] =
  traversal.flatMap(_.closureBindingId)
...
import Accessors.*
implicit def accessPropertyAliasTypeFullNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasAliasTypeFullNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_ALIAS_TYPE_FULL_NAME[NodeType] = new Traversal_Property_ALIAS_TYPE_FULL_NAME(traversal.iterator)
implicit def accessPropertyArgumentIndexTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasArgumentIndexEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_ARGUMENT_INDEX[NodeType] = new Traversal_Property_ARGUMENT_INDEX(traversal.iterator)
implicit def accessPropertyArgumentNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasArgumentNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_ARGUMENT_NAME[NodeType] = new Traversal_Property_ARGUMENT_NAME(traversal.iterator)
implicit def accessPropertyAstParentFullNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasAstParentFullNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_AST_PARENT_FULL_NAME[NodeType] = new Traversal_Property_AST_PARENT_FULL_NAME(traversal.iterator)
implicit def accessPropertyAstParentTypeTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasAstParentTypeEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_AST_PARENT_TYPE[NodeType] = new Traversal_Property_AST_PARENT_TYPE(traversal.iterator)
implicit def accessPropertyCanonicalNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasCanonicalNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_CANONICAL_NAME[NodeType] = new Traversal_Property_CANONICAL_NAME(traversal.iterator)
implicit def accessPropertyClassNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasClassNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_CLASS_NAME[NodeType] = new Traversal_Property_CLASS_NAME(traversal.iterator)
implicit def accessPropertyClassShortNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasClassShortNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_CLASS_SHORT_NAME[NodeType] = new Traversal_Property_CLASS_SHORT_NAME(traversal.iterator)
implicit def accessPropertyClosureBindingIdTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasClosureBindingIdEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_CLOSURE_BINDING_ID[NodeType] = new Traversal_Property_CLOSURE_BINDING_ID(traversal.iterator)
implicit def accessPropertyClosureOriginalNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasClosureOriginalNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_CLOSURE_ORIGINAL_NAME[NodeType] = new Traversal_Property_CLOSURE_ORIGINAL_NAME(traversal.iterator)
implicit def accessPropertyCodeTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasCodeEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_CODE[NodeType] = new Traversal_Property_CODE(traversal.iterator)
implicit def accessPropertyColumnNumberTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasColumnNumberEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_COLUMN_NUMBER[NodeType] = new Traversal_Property_COLUMN_NUMBER(traversal.iterator)
implicit def accessPropertyColumnNumberEndTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasColumnNumberEndEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_COLUMN_NUMBER_END[NodeType] = new Traversal_Property_COLUMN_NUMBER_END(traversal.iterator)
implicit def accessPropertyContainedRefTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasContainedRefEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_CONTAINED_REF[NodeType] = new Traversal_Property_CONTAINED_REF(traversal.iterator)
implicit def accessPropertyContentTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasContentEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_CONTENT[NodeType] = new Traversal_Property_CONTENT(traversal.iterator)
implicit def accessPropertyControlStructureTypeTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasControlStructureTypeEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_CONTROL_STRUCTURE_TYPE[NodeType] = new Traversal_Property_CONTROL_STRUCTURE_TYPE(traversal.iterator)
implicit def accessPropertyDependencyGroupIdTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasDependencyGroupIdEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_DEPENDENCY_GROUP_ID[NodeType] = new Traversal_Property_DEPENDENCY_GROUP_ID(traversal.iterator)
implicit def accessPropertyDispatchTypeTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasDispatchTypeEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_DISPATCH_TYPE[NodeType] = new Traversal_Property_DISPATCH_TYPE(traversal.iterator)
implicit def accessPropertyDynamicTypeHintFullNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasDynamicTypeHintFullNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_DYNAMIC_TYPE_HINT_FULL_NAME[NodeType] = new Traversal_Property_DYNAMIC_TYPE_HINT_FULL_NAME(traversal.iterator)
implicit def accessPropertyEvaluationStrategyTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasEvaluationStrategyEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_EVALUATION_STRATEGY[NodeType] = new Traversal_Property_EVALUATION_STRATEGY(traversal.iterator)
implicit def accessPropertyExplicitAsTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasExplicitAsEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_EXPLICIT_AS[NodeType] = new Traversal_Property_EXPLICIT_AS(traversal.iterator)
implicit def accessPropertyFilenameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasFilenameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_FILENAME[NodeType] = new Traversal_Property_FILENAME(traversal.iterator)
implicit def accessPropertyFullNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasFullNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_FULL_NAME[NodeType] = new Traversal_Property_FULL_NAME(traversal.iterator)
implicit def accessPropertyHashTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasHashEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_HASH[NodeType] = new Traversal_Property_HASH(traversal.iterator)
implicit def accessPropertyImportedAsTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasImportedAsEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_IMPORTED_AS[NodeType] = new Traversal_Property_IMPORTED_AS(traversal.iterator)
implicit def accessPropertyImportedEntityTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasImportedEntityEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_IMPORTED_ENTITY[NodeType] = new Traversal_Property_IMPORTED_ENTITY(traversal.iterator)
implicit def accessPropertyIndexTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasIndexEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_INDEX[NodeType] = new Traversal_Property_INDEX(traversal.iterator)
implicit def accessPropertyInheritsFromTypeFullNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasInheritsFromTypeFullNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_INHERITS_FROM_TYPE_FULL_NAME[NodeType] = new Traversal_Property_INHERITS_FROM_TYPE_FULL_NAME(traversal.iterator)
implicit def accessPropertyIsExplicitTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasIsExplicitEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_IS_EXPLICIT[NodeType] = new Traversal_Property_IS_EXPLICIT(traversal.iterator)
implicit def accessPropertyIsExternalTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasIsExternalEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_IS_EXTERNAL[NodeType] = new Traversal_Property_IS_EXTERNAL(traversal.iterator)
implicit def accessPropertyIsVariadicTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasIsVariadicEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_IS_VARIADIC[NodeType] = new Traversal_Property_IS_VARIADIC(traversal.iterator)
implicit def accessPropertyIsWildcardTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasIsWildcardEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_IS_WILDCARD[NodeType] = new Traversal_Property_IS_WILDCARD(traversal.iterator)
implicit def accessPropertyKeyTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasKeyEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_KEY[NodeType] = new Traversal_Property_KEY(traversal.iterator)
implicit def accessPropertyLanguageTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasLanguageEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_LANGUAGE[NodeType] = new Traversal_Property_LANGUAGE(traversal.iterator)
implicit def accessPropertyLineNumberTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasLineNumberEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_LINE_NUMBER[NodeType] = new Traversal_Property_LINE_NUMBER(traversal.iterator)
implicit def accessPropertyLineNumberEndTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasLineNumberEndEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_LINE_NUMBER_END[NodeType] = new Traversal_Property_LINE_NUMBER_END(traversal.iterator)
implicit def accessPropertyMethodFullNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasMethodFullNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_METHOD_FULL_NAME[NodeType] = new Traversal_Property_METHOD_FULL_NAME(traversal.iterator)
implicit def accessPropertyMethodShortNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasMethodShortNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_METHOD_SHORT_NAME[NodeType] = new Traversal_Property_METHOD_SHORT_NAME(traversal.iterator)
implicit def accessPropertyModifierTypeTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasModifierTypeEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_MODIFIER_TYPE[NodeType] = new Traversal_Property_MODIFIER_TYPE(traversal.iterator)
implicit def accessPropertyNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_NAME[NodeType] = new Traversal_Property_NAME(traversal.iterator)
implicit def accessPropertyNodeLabelTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasNodeLabelEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_NODE_LABEL[NodeType] = new Traversal_Property_NODE_LABEL(traversal.iterator)
implicit def accessPropertyOffsetTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasOffsetEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_OFFSET[NodeType] = new Traversal_Property_OFFSET(traversal.iterator)
implicit def accessPropertyOffsetEndTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasOffsetEndEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_OFFSET_END[NodeType] = new Traversal_Property_OFFSET_END(traversal.iterator)
implicit def accessPropertyOrderTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasOrderEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_ORDER[NodeType] = new Traversal_Property_ORDER(traversal.iterator)
implicit def accessPropertyOverlaysTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasOverlaysEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_OVERLAYS[NodeType] = new Traversal_Property_OVERLAYS(traversal.iterator)
implicit def accessPropertyPackageNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasPackageNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_PACKAGE_NAME[NodeType] = new Traversal_Property_PACKAGE_NAME(traversal.iterator)
implicit def accessPropertyParserTypeNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasParserTypeNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_PARSER_TYPE_NAME[NodeType] = new Traversal_Property_PARSER_TYPE_NAME(traversal.iterator)
implicit def accessPropertyPossibleTypesTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasPossibleTypesEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_POSSIBLE_TYPES[NodeType] = new Traversal_Property_POSSIBLE_TYPES(traversal.iterator)
implicit def accessPropertyRootTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasRootEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_ROOT[NodeType] = new Traversal_Property_ROOT(traversal.iterator)
implicit def accessPropertySignatureTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasSignatureEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_SIGNATURE[NodeType] = new Traversal_Property_SIGNATURE(traversal.iterator)
implicit def accessPropertySymbolTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasSymbolEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_SYMBOL[NodeType] = new Traversal_Property_SYMBOL(traversal.iterator)
implicit def accessPropertyTypeDeclFullNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasTypeDeclFullNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_TYPE_DECL_FULL_NAME[NodeType] = new Traversal_Property_TYPE_DECL_FULL_NAME(traversal.iterator)
implicit def accessPropertyTypeFullNameTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasTypeFullNameEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_TYPE_FULL_NAME[NodeType] = new Traversal_Property_TYPE_FULL_NAME(traversal.iterator)
implicit def accessPropertyValueTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasValueEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_VALUE[NodeType] = new Traversal_Property_VALUE(traversal.iterator)
implicit def accessPropertyVersionTraversal[NodeType <: nodes.StoredNode with nodes.StaticType[nodes.HasVersionEMT]](traversal: IterableOnce[NodeType]): Traversal_Property_VERSION[NodeType] = new Traversal_Property_VERSION(traversal.iterator)
}

trait ConcreteBaseConversions  extends AbstractBaseConversions0 {
import Accessors.*
implicit def traversal_AnnotationBase[NodeType <: nodes.AnnotationBase](traversal: IterableOnce[NodeType]): Traversal_AnnotationBase[NodeType] = new Traversal_AnnotationBase(traversal.iterator)
implicit def traversal_AnnotationLiteralBase[NodeType <: nodes.AnnotationLiteralBase](traversal: IterableOnce[NodeType]): Traversal_AnnotationLiteralBase[NodeType] = new Traversal_AnnotationLiteralBase(traversal.iterator)
implicit def traversal_AnnotationParameterBase[NodeType <: nodes.AnnotationParameterBase](traversal: IterableOnce[NodeType]): Traversal_AnnotationParameterBase[NodeType] = new Traversal_AnnotationParameterBase(traversal.iterator)
implicit def traversal_AnnotationParameterAssignBase[NodeType <: nodes.AnnotationParameterAssignBase](traversal: IterableOnce[NodeType]): Traversal_AnnotationParameterAssignBase[NodeType] = new Traversal_AnnotationParameterAssignBase(traversal.iterator)
implicit def traversal_ArrayInitializerBase[NodeType <: nodes.ArrayInitializerBase](traversal: IterableOnce[NodeType]): Traversal_ArrayInitializerBase[NodeType] = new Traversal_ArrayInitializerBase(traversal.iterator)
implicit def traversal_BindingBase[NodeType <: nodes.BindingBase](traversal: IterableOnce[NodeType]): Traversal_BindingBase[NodeType] = new Traversal_BindingBase(traversal.iterator)
implicit def traversal_BlockBase[NodeType <: nodes.BlockBase](traversal: IterableOnce[NodeType]): Traversal_BlockBase[NodeType] = new Traversal_BlockBase(traversal.iterator)
implicit def traversal_CallBase[NodeType <: nodes.CallBase](traversal: IterableOnce[NodeType]): Traversal_CallBase[NodeType] = new Traversal_CallBase(traversal.iterator)
implicit def traversal_ClosureBindingBase[NodeType <: nodes.ClosureBindingBase](traversal: IterableOnce[NodeType]): Traversal_ClosureBindingBase[NodeType] = new Traversal_ClosureBindingBase(traversal.iterator)
implicit def traversal_CommentBase[NodeType <: nodes.CommentBase](traversal: IterableOnce[NodeType]): Traversal_CommentBase[NodeType] = new Traversal_CommentBase(traversal.iterator)
implicit def traversal_ConfigFileBase[NodeType <: nodes.ConfigFileBase](traversal: IterableOnce[NodeType]): Traversal_ConfigFileBase[NodeType] = new Traversal_ConfigFileBase(traversal.iterator)
implicit def traversal_ControlStructureBase[NodeType <: nodes.ControlStructureBase](traversal: IterableOnce[NodeType]): Traversal_ControlStructureBase[NodeType] = new Traversal_ControlStructureBase(traversal.iterator)
implicit def traversal_DependencyBase[NodeType <: nodes.DependencyBase](traversal: IterableOnce[NodeType]): Traversal_DependencyBase[NodeType] = new Traversal_DependencyBase(traversal.iterator)
implicit def traversal_FieldIdentifierBase[NodeType <: nodes.FieldIdentifierBase](traversal: IterableOnce[NodeType]): Traversal_FieldIdentifierBase[NodeType] = new Traversal_FieldIdentifierBase(traversal.iterator)
implicit def traversal_FileBase[NodeType <: nodes.FileBase](traversal: IterableOnce[NodeType]): Traversal_FileBase[NodeType] = new Traversal_FileBase(traversal.iterator)
implicit def traversal_FindingBase[NodeType <: nodes.FindingBase](traversal: IterableOnce[NodeType]): Traversal_FindingBase[NodeType] = new Traversal_FindingBase(traversal.iterator)
implicit def traversal_IdentifierBase[NodeType <: nodes.IdentifierBase](traversal: IterableOnce[NodeType]): Traversal_IdentifierBase[NodeType] = new Traversal_IdentifierBase(traversal.iterator)
implicit def traversal_ImportBase[NodeType <: nodes.ImportBase](traversal: IterableOnce[NodeType]): Traversal_ImportBase[NodeType] = new Traversal_ImportBase(traversal.iterator)
implicit def traversal_JumpLabelBase[NodeType <: nodes.JumpLabelBase](traversal: IterableOnce[NodeType]): Traversal_JumpLabelBase[NodeType] = new Traversal_JumpLabelBase(traversal.iterator)
implicit def traversal_JumpTargetBase[NodeType <: nodes.JumpTargetBase](traversal: IterableOnce[NodeType]): Traversal_JumpTargetBase[NodeType] = new Traversal_JumpTargetBase(traversal.iterator)
implicit def traversal_KeyValuePairBase[NodeType <: nodes.KeyValuePairBase](traversal: IterableOnce[NodeType]): Traversal_KeyValuePairBase[NodeType] = new Traversal_KeyValuePairBase(traversal.iterator)
implicit def traversal_LiteralBase[NodeType <: nodes.LiteralBase](traversal: IterableOnce[NodeType]): Traversal_LiteralBase[NodeType] = new Traversal_LiteralBase(traversal.iterator)
implicit def traversal_LocalBase[NodeType <: nodes.LocalBase](traversal: IterableOnce[NodeType]): Traversal_LocalBase[NodeType] = new Traversal_LocalBase(traversal.iterator)
implicit def traversal_LocationBase[NodeType <: nodes.LocationBase](traversal: IterableOnce[NodeType]): Traversal_LocationBase[NodeType] = new Traversal_LocationBase(traversal.iterator)
implicit def traversal_MemberBase[NodeType <: nodes.MemberBase](traversal: IterableOnce[NodeType]): Traversal_MemberBase[NodeType] = new Traversal_MemberBase(traversal.iterator)
implicit def traversal_MetaDataBase[NodeType <: nodes.MetaDataBase](traversal: IterableOnce[NodeType]): Traversal_MetaDataBase[NodeType] = new Traversal_MetaDataBase(traversal.iterator)
implicit def traversal_MethodBase[NodeType <: nodes.MethodBase](traversal: IterableOnce[NodeType]): Traversal_MethodBase[NodeType] = new Traversal_MethodBase(traversal.iterator)
implicit def traversal_MethodParameterInBase[NodeType <: nodes.MethodParameterInBase](traversal: IterableOnce[NodeType]): Traversal_MethodParameterInBase[NodeType] = new Traversal_MethodParameterInBase(traversal.iterator)
implicit def traversal_MethodParameterOutBase[NodeType <: nodes.MethodParameterOutBase](traversal: IterableOnce[NodeType]): Traversal_MethodParameterOutBase[NodeType] = new Traversal_MethodParameterOutBase(traversal.iterator)
implicit def traversal_MethodRefBase[NodeType <: nodes.MethodRefBase](traversal: IterableOnce[NodeType]): Traversal_MethodRefBase[NodeType] = new Traversal_MethodRefBase(traversal.iterator)
implicit def traversal_MethodReturnBase[NodeType <: nodes.MethodReturnBase](traversal: IterableOnce[NodeType]): Traversal_MethodReturnBase[NodeType] = new Traversal_MethodReturnBase(traversal.iterator)
implicit def traversal_ModifierBase[NodeType <: nodes.ModifierBase](traversal: IterableOnce[NodeType]): Traversal_ModifierBase[NodeType] = new Traversal_ModifierBase(traversal.iterator)
implicit def traversal_NamespaceBase[NodeType <: nodes.NamespaceBase](traversal: IterableOnce[NodeType]): Traversal_NamespaceBase[NodeType] = new Traversal_NamespaceBase(traversal.iterator)
implicit def traversal_NamespaceBlockBase[NodeType <: nodes.NamespaceBlockBase](traversal: IterableOnce[NodeType]): Traversal_NamespaceBlockBase[NodeType] = new Traversal_NamespaceBlockBase(traversal.iterator)
implicit def traversal_ReturnBase[NodeType <: nodes.ReturnBase](traversal: IterableOnce[NodeType]): Traversal_ReturnBase[NodeType] = new Traversal_ReturnBase(traversal.iterator)
implicit def traversal_TagBase[NodeType <: nodes.TagBase](traversal: IterableOnce[NodeType]): Traversal_TagBase[NodeType] = new Traversal_TagBase(traversal.iterator)
implicit def traversal_TagNodePairBase[NodeType <: nodes.TagNodePairBase](traversal: IterableOnce[NodeType]): Traversal_TagNodePairBase[NodeType] = new Traversal_TagNodePairBase(traversal.iterator)
implicit def traversal_TemplateDomBase[NodeType <: nodes.TemplateDomBase](traversal: IterableOnce[NodeType]): Traversal_TemplateDomBase[NodeType] = new Traversal_TemplateDomBase(traversal.iterator)
implicit def traversal_TypeBase[NodeType <: nodes.TypeBase](traversal: IterableOnce[NodeType]): Traversal_TypeBase[NodeType] = new Traversal_TypeBase(traversal.iterator)
implicit def traversal_TypeArgumentBase[NodeType <: nodes.TypeArgumentBase](traversal: IterableOnce[NodeType]): Traversal_TypeArgumentBase[NodeType] = new Traversal_TypeArgumentBase(traversal.iterator)
implicit def traversal_TypeDeclBase[NodeType <: nodes.TypeDeclBase](traversal: IterableOnce[NodeType]): Traversal_TypeDeclBase[NodeType] = new Traversal_TypeDeclBase(traversal.iterator)
implicit def traversal_TypeParameterBase[NodeType <: nodes.TypeParameterBase](traversal: IterableOnce[NodeType]): Traversal_TypeParameterBase[NodeType] = new Traversal_TypeParameterBase(traversal.iterator)
implicit def traversal_TypeRefBase[NodeType <: nodes.TypeRefBase](traversal: IterableOnce[NodeType]): Traversal_TypeRefBase[NodeType] = new Traversal_TypeRefBase(traversal.iterator)
implicit def traversal_UnknownBase[NodeType <: nodes.UnknownBase](traversal: IterableOnce[NodeType]): Traversal_UnknownBase[NodeType] = new Traversal_UnknownBase(traversal.iterator)
}

trait AbstractBaseConversions0  extends AbstractBaseConversions1 {
import Accessors.*
implicit def traversal_AstNodeBase[NodeType <: nodes.AstNodeBase](traversal: IterableOnce[NodeType]): Traversal_AstNodeBase[NodeType] = new Traversal_AstNodeBase(traversal.iterator)
implicit def traversal_CallReprBase[NodeType <: nodes.CallReprBase](traversal: IterableOnce[NodeType]): Traversal_CallReprBase[NodeType] = new Traversal_CallReprBase(traversal.iterator)
implicit def traversal_CfgNodeBase[NodeType <: nodes.CfgNodeBase](traversal: IterableOnce[NodeType]): Traversal_CfgNodeBase[NodeType] = new Traversal_CfgNodeBase(traversal.iterator)
implicit def traversal_ExpressionBase[NodeType <: nodes.ExpressionBase](traversal: IterableOnce[NodeType]): Traversal_ExpressionBase[NodeType] = new Traversal_ExpressionBase(traversal.iterator)
}

trait AbstractBaseConversions1  {
import Accessors.*
implicit def traversal_DeclarationBase[NodeType <: nodes.DeclarationBase](traversal: IterableOnce[NodeType]): Traversal_DeclarationBase[NodeType] = new Traversal_DeclarationBase(traversal.iterator)
}
