...
def callViaArgumentIn: nodes.Call = {
  try { argumentIn.collectAll[nodes.Call].next() } catch {
    case e: java.util.NoSuchElementException =>
      throw new flatgraph.SchemaViolationException("IN edge with label ARGUMENT to an adjacent CALL is mandatory, but not defined for this FIELD_IDENTIFIER node with seq=" + node.seq, e)
  }
}
...
def callViaAstIn: nodes.Call = {
  try { astIn.collectAll[nodes.Call].next() } catch {
    case e: java.util.NoSuchElementException =>
      throw new flatgraph.SchemaViolationException("IN edge with label AST to an adjacent CALL is mandatory, but not defined for this FIELD_IDENTIFIER node with seq=" + node.seq, e)
  }
}
...
def callViaCfgOut: nodes.Call = {
  try { cfgOut.collectAll[nodes.Call].next() } catch {
    case e: java.util.NoSuchElementException =>
      throw new flatgraph.SchemaViolationException("OUT edge with label CFG to an adjacent CALL is mandatory, but not defined for this FIELD_IDENTIFIER node with seq=" + node.seq, e)
  }
}
...
/** 
 * Traverse to CALL via ARGUMENT IN edge. */
def callViaArgumentIn: Iterator[nodes.Call] = traversal.map(_.callViaArgumentIn)


/** 
 * Traverse to CALL via AST IN edge. */
def callViaAstIn: Iterator[nodes.Call] = traversal.map(_.callViaAstIn)
...
/** 
 * Traverse to CALL via CFG OUT edge. */
def callViaCfgOut: Iterator[nodes.Call] = traversal.map(_.callViaCfgOut)
