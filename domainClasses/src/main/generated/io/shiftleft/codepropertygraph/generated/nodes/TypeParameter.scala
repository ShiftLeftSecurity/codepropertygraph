...
class TypeParameter(graph_4762: Graph, id_4762: Long /*cf https://github.com/scala/bug/issues/4762 */ )
  ...
  /** Traverse to TYPE_ARGUMENT via BINDS_TO IN edge.
    */
  def typeArgumentViaBindsToIn: overflowdb.traversal.Traversal[TypeArgument] = get().typeArgumentViaBindsToIn

  @deprecated("please use `typeArgumentViaBindsToIn`", "June 2024")
  def _typeArgumentViaBindsToIn = typeArgumentViaBindsToIn
...
class TypeParameterDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode with AstNode with TypeParameterBase {
  ...
  def typeArgumentViaBindsToIn: overflowdb.traversal.Traversal[TypeArgument] = bindsToIn.collectAll[TypeArgument]
