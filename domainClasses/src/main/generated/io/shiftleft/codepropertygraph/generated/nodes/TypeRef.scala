...
class TypeRefDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode with Expression with TypeRefBase {
  ...
  def captureOut: Iterator[ClosureBinding] = createAdjacentNodeScalaIteratorByOffSet[ClosureBinding](7)
  override def _captureOut                 = createAdjacentNodeScalaIteratorByOffSet[StoredNode](7)
  def _closureBindingViaCaptureOut: overflowdb.traversal.Traversal[ClosureBinding] =
    captureOut.collectAll[ClosureBinding]

  def cfgOut: Iterator[AstNode] = createAdjacentNodeScalaIteratorByOffSet[AstNode](8)
