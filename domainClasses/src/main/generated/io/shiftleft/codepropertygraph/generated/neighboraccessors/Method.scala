...
def methodParameterInViaReachingDefOut: Iterator[nodes.MethodParameterIn] = reachingDefOut.collectAll[nodes.MethodParameterIn]
 ...
 * Traverse to METHOD_RETURN via AST OUT edge. */
...
def methodReturn: nodes.MethodReturn = {
  try { astOut.collectAll[nodes.MethodReturn].next() } catch {
    case e: java.util.NoSuchElementException =>
      throw new flatgraph.SchemaViolationException("OUT edge with label AST to an adjacent METHOD_RETURN is mandatory, but not defined for this METHOD node with seq=" + node.seq, e)
  }
}


/** Literals used in the method
 * Traverse to LITERAL via CONTAINS OUT edge. */
...
def block: nodes.Block = {
  try { astOut.collectAll[nodes.Block].next() } catch {
    case e: java.util.NoSuchElementException =>
      throw new flatgraph.SchemaViolationException("OUT edge with label AST to an adjacent BLOCK is mandatory, but not defined for this METHOD node with seq=" + node.seq, e)
  }
}
...
/** 
 * Traverse to CONTROL_STRUCTURE via CONTAINS OUT edge. */
def controlStructureViaContainsOut: Iterator[nodes.ControlStructure] = traversal.flatMap(_.controlStructureViaContainsOut)
...
/** 
 * Traverse to FIELD_IDENTIFIER via CONTAINS OUT edge. */
def fieldIdentifierViaContainsOut: Iterator[nodes.FieldIdentifier] = traversal.flatMap(_.fieldIdentifierViaContainsOut)
...
/** 
 * Traverse to METHOD_PARAMETER_IN via REACHING_DEF OUT edge. */
def methodParameterInViaReachingDefOut: Iterator[nodes.MethodParameterIn] = traversal.flatMap(_.methodParameterInViaReachingDefOut)
