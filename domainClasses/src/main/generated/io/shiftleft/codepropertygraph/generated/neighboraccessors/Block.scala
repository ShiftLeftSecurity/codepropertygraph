...
def blockViaAstIn: nodes.Block = {
  try { astIn.collectAll[nodes.Block].next() } catch {
    case e: java.util.NoSuchElementException =>
      throw new flatgraph.SchemaViolationException("IN edge with label AST to an adjacent BLOCK is mandatory, but not defined for this BLOCK node with seq=" + node.seq, e)
  }
}
...
def methodViaAstIn: nodes.Method = {
  try { astIn.collectAll[nodes.Method].next() } catch {
    case e: java.util.NoSuchElementException =>
      throw new flatgraph.SchemaViolationException("IN edge with label AST to an adjacent METHOD is mandatory, but not defined for this BLOCK node with seq=" + node.seq, e)
  }
}
...
final class AccessNeighborsForBlockTraversal(val traversal: Iterator[nodes.Block]) extends AnyVal {
  /** 
 * Traverse to BLOCK via AST IN edge. */
def blockViaAstIn: Iterator[nodes.Block] = traversal.map(_.blockViaAstIn)
...
/** 
 * Traverse to BLOCK via REACHING_DEF OUT edge. */
def blockViaReachingDefOut: Iterator[nodes.Block] = traversal.flatMap(_.blockViaReachingDefOut)
