...
def closureBindingViaCaptureOut: Iterator[nodes.ClosureBinding] = captureOut.collectAll[nodes.ClosureBinding]


/** 
 * Traverse to CONTROL_STRUCTURE via AST IN edge. */
def controlStructureViaAstIn: nodes.ControlStructure = {
  try { astIn.collectAll[nodes.ControlStructure].next() } catch {
    case e: java.util.NoSuchElementException =>
      throw new flatgraph.SchemaViolationException("IN edge with label AST to an adjacent CONTROL_STRUCTURE is mandatory, but not defined for this METHOD_REF node with seq=" + node.seq, e)
  }
}
...
def methodViaRefOut: nodes.Method           = referencedMethod


/** Traverse to referenced method.
 * Traverse to METHOD via REF OUT edge. */
def referencedMethod: nodes.Method = {
  try { refOut.collectAll[nodes.Method].next() } catch {
    case e: java.util.NoSuchElementException =>
      throw new flatgraph.SchemaViolationException("OUT edge with label REF to an adjacent METHOD is mandatory, but not defined for this METHOD_REF node with seq=" + node.seq, e)
  }
}
...
/** 
 * Traverse to CLOSURE_BINDING via CAPTURE OUT edge. */
def closureBindingViaCaptureOut: Iterator[nodes.ClosureBinding] = traversal.flatMap(_.closureBindingViaCaptureOut)
