...
object Unknown {
  ...
  object PropertyNames {
    ...
    val all: Set[String] = Set(ArgumentIndex, Code, ColumnNumber, DepthFirstOrder, DynamicTypeHintFullName, InternalFlags, LineNumber, Order, ParserTypeName, TypeFullName)
  ...
  object Edges {
    val Out: Array[String] = Array("AST","CDG","CFG","DOMINATE","DYNAMIC_TYPE","EVAL_TYPE","POST_DOMINATE","REACHING_DEF","TAGGED_BY")
...
trait UnknownBase extends CpgNode with ExpressionBase with HasArgumentIndex with HasCode with HasColumnNumber with HasDepthFirstOrder with HasDynamicTypeHintFullName with HasInternalFlags with HasLineNumber with HasOrder with HasParserTypeName with HasTypeFullName {
  def asStored : StoredNode = this.asInstanceOf[StoredNode]

  
}

class Unknown(graph: Graph, id: Long) extends NodeRef[UnknownDb](graph, id)
  with UnknownBase
  with StoredNode
  with Expression {
...
class UnknownDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
...
override def fromNewNode(someNewNode: NewNode, mapping: NewNode => StoredNode):Unit = {
  ...
  val other = someNewNode.asInstanceOf[NewUnknown]
...
class UnknownTraversal[NodeType <: Unknown](val traversal: Traversal[NodeType]) extends AnyVal {
