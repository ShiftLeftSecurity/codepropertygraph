...
object Identifier {
  def apply(graph: Graph, id: Long) = new Identifier(graph, id)
  ...
  object PropertyNames {
    ...
    val all: Set[String] = Set(ArgumentIndex, Code, ColumnNumber, DepthFirstOrder, DynamicTypeHintFullName, InternalFlags, LineNumber, Name, Order, TypeFullName)
  ...
  object Edges {
    val Out: Array[String] = Array("CDG","CFG","DOMINATE","DYNAMIC_TYPE","EVAL_TYPE","POST_DOMINATE","REACHING_DEF","REF","TAGGED_BY")
  ...
  val factory = new NodeFactory[IdentifierDb] {
    ...
    override def createNode(ref: NodeRef[IdentifierDb]) =
      new IdentifierDb(ref.asInstanceOf[NodeRef[NodeDb]])

    override def createNodeRef(graph: Graph, id: Long) = Identifier(graph, id)
  }
}

trait IdentifierBase extends CpgNode with LocalLikeBase with ExpressionBase with HasArgumentIndex with HasCode with HasColumnNumber with HasDepthFirstOrder with HasDynamicTypeHintFullName with HasInternalFlags with HasLineNumber with HasName with HasOrder with HasTypeFullName {
  def asStored : StoredNode = this.asInstanceOf[StoredNode]

  
}

class Identifier(graph: Graph, id: Long) extends NodeRef[IdentifierDb](graph, id)
  with IdentifierBase
  with StoredNode
  with LocalLike with Expression {
...
class IdentifierDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
  with LocalLike with Expression with IdentifierBase {
...
if (typeFullName != null) { properties.put("TYPE_FULL_NAME", typeFullName) }
  ...
  def _methodParameterInViaRefOut: Option[MethodParameterIn] = _refOut.asScala.collect { case node: MethodParameterIn => node }.nextOption()
def _localViaRefOut: Option[Local] = _refOut.asScala.collect { case node: Local => node }.nextOption()
...
override def _taggedByOut: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(7).asInstanceOf[JIterator[StoredNode]]
...
override def fromNewNode(someNewNode: NewNode, mapping: NewNode => StoredNode):Unit = {
  ...
  val other = someNewNode.asInstanceOf[NewIdentifier]
...
class IdentifierTraversal[NodeType <: Identifier](val traversal: Traversal[NodeType]) extends AnyVal {
