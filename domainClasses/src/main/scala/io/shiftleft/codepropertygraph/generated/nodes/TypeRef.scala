...
object TypeRef {
  ...
  val layoutInformation = new NodeLayoutInformation(
    ...
    List(edges.Capture.layoutInformation, edges.Dominate.layoutInformation, edges.Cdg.layoutInformation, edges.EvalType.layoutInformation, edges.Cfg.layoutInformation, edges.PostDominate.layoutInformation, edges.DynamicType.layoutInformation).asJava,
  ...
  object Edges {
    val Out: Array[String] = Array("CAPTURE","CDG","CFG","DOMINATE","DYNAMIC_TYPE","EVAL_TYPE","POST_DOMINATE")
...
trait TypeRefBase extends CpgNode with ExpressionBase with HasArgumentIndex with HasCode with HasColumnNumber with HasDepthFirstOrder with HasDynamicTypeHintFullName with HasInternalFlags with HasLineNumber with HasOrder with HasTypeFullName {
  def asStored : StoredNode = this.asInstanceOf[StoredNode]

  
}

class TypeRef(graph: Graph, id: Long) extends NodeRef[TypeRefDb](graph, id)
  with TypeRefBase
  with StoredNode
  with Expression {
...
class TypeRefDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
  with Expression with TypeRefBase {
...
override def _dominateOut: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(1).asInstanceOf[JIterator[StoredNode]]
...
override def _dynamicTypeOut: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(6).asInstanceOf[JIterator[StoredNode]]
...
override def _dominateIn: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(8).asInstanceOf[JIterator[StoredNode]]
...
override def _conditionIn: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(10).asInstanceOf[JIterator[StoredNode]]
...
override def _containsIn: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(12).asInstanceOf[JIterator[StoredNode]]
...
override def _cfgIn: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(13).asInstanceOf[JIterator[StoredNode]]
...
override def fromNewNode(someNewNode: NewNode, mapping: NewNode => StoredNode):Unit = {
  ...
  val other = someNewNode.asInstanceOf[NewTypeRef]
...
class TypeRefTraversal[NodeType <: TypeRef](val traversal: Traversal[NodeType]) extends AnyVal {
