...
object PostExecutionCall {
  def apply(graph: Graph, id: Long) = new PostExecutionCall(graph, id)

  val Label = "POST_EXECUTION_CALL"
  ...
  val factory = new NodeFactory[PostExecutionCallDb] {
    override val forLabel = PostExecutionCall.Label

    override def createNode(ref: NodeRef[PostExecutionCallDb]) =
      new PostExecutionCallDb(ref.asInstanceOf[NodeRef[NodeDb]])

    override def createNodeRef(graph: Graph, id: Long) = PostExecutionCall(graph, id)
  }
}

trait PostExecutionCallBase extends CpgNode with TrackingPointBase with CallReprBase with HasCode with HasColumnNumber with HasDepthFirstOrder with HasInternalFlags with HasLineNumber with HasName with HasOrder with HasSignature {
  def asStored : StoredNode = this.asInstanceOf[StoredNode]

  
}

class PostExecutionCall(graph: Graph, id: Long) extends NodeRef[PostExecutionCallDb](graph, id)
  ...
  def _methodReturnViaRefOut: Iterator[MethodReturn] = get()._methodReturnViaRefOut
def _methodParameterOutViaRefOut: Iterator[MethodParameterOut] = get()._methodParameterOutViaRefOut
...
case 8 => signature
  ...
  override def productPrefix = "PostExecutionCall"
  override def productArity = 9
}

class PostExecutionCallDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
  with TrackingPoint with CallRepr with PostExecutionCallBase {

  override def layoutInformation: NodeLayoutInformation = PostExecutionCall.layoutInformation
...
if (signature != null) { properties.put("SIGNATURE", signature) }
  ...
  def _methodReturnViaRefOut: Iterator[MethodReturn] = _refOut.asScala.collect { case node: MethodReturn => node }
def _methodParameterOutViaRefOut: Iterator[MethodParameterOut] = _refOut.asScala.collect { case node: MethodParameterOut => node }
...
case 8 => signature
  ...
  override def canEqual(that: Any): Boolean = that != null && that.isInstanceOf[PostExecutionCallDb]
...
override def fromNewNode(someNewNode: NewNode, mapping: NewNode => StoredNode):Unit = {
  ...
  val other = someNewNode.asInstanceOf[NewPostExecutionCall]
...
class PostExecutionCallTraversal[NodeType <: PostExecutionCall](val traversal: Traversal[NodeType]) extends AnyVal {
