...
object Block {
  def apply(graph: Graph, id: Long) = new Block(graph, id)
  ...
  object PropertyNames {
    ...
    val all: Set[String] = Set(ArgumentIndex, Code, ColumnNumber, DepthFirstOrder, DynamicTypeHintFullName, InternalFlags, LineNumber, Order, TypeFullName)
  ...
  val layoutInformation = new NodeLayoutInformation(
    ...
    List(edges.Dominate.layoutInformation, edges.Ast.layoutInformation, edges.Cdg.layoutInformation, edges.EvalType.layoutInformation, edges.Cfg.layoutInformation, edges.PostDominate.layoutInformation, edges.ReachingDef.layoutInformation, edges.DynamicType.layoutInformation).asJava,
  ...
  object Edges {
    val Out: Array[String] = Array("AST","CDG","CFG","DOMINATE","DYNAMIC_TYPE","EVAL_TYPE","POST_DOMINATE","REACHING_DEF")
  ...
  val factory = new NodeFactory[BlockDb] {
    ...
    override def createNode(ref: NodeRef[BlockDb]) =
      new BlockDb(ref.asInstanceOf[NodeRef[NodeDb]])

    override def createNodeRef(graph: Graph, id: Long) = Block(graph, id)
  }
}

trait BlockBase extends CpgNode with ExpressionBase with HasArgumentIndex with HasCode with HasColumnNumber with HasDepthFirstOrder with HasDynamicTypeHintFullName with HasInternalFlags with HasLineNumber with HasOrder with HasTypeFullName {
  def asStored : StoredNode = this.asInstanceOf[StoredNode]

  
}

class Block(graph: Graph, id: Long) extends NodeRef[BlockDb](graph, id)
  with BlockBase
  with StoredNode
  with Expression {
...
def _blockViaReachingDefOut: Iterator[Block] = get()._blockViaReachingDefOut
...
def _blockViaAstIn: Block = get()._blockViaAstIn
...
class BlockDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
...
override def _cdgOut: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(2).asInstanceOf[JIterator[StoredNode]]
...
def _blockViaReachingDefOut: Iterator[Block] = _reachingDefOut.asScala.collect { case node: Block => node }
def _literalViaReachingDefOut: Iterator[Literal] = _reachingDefOut.asScala.collect { case node: Literal => node }
override def _reachingDefOut: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(6).asInstanceOf[JIterator[StoredNode]]
...
def _blockViaAstIn: Block = _astIn.asScala.collect { case node: Block => node }.next()
...
override def _cdgIn: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(12).asInstanceOf[JIterator[StoredNode]]
...
override def _argumentIn: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(15).asInstanceOf[JIterator[StoredNode]]
...
override def _reachingDefIn: JIterator[StoredNode] = createAdjacentNodeIteratorByOffSet(17).asInstanceOf[JIterator[StoredNode]]
...
override def fromNewNode(someNewNode: NewNode, mapping: NewNode => StoredNode):Unit = {
  ...
  val other = someNewNode.asInstanceOf[NewBlock]
...
class BlockTraversal[NodeType <: Block](val traversal: Traversal[NodeType]) extends AnyVal {
