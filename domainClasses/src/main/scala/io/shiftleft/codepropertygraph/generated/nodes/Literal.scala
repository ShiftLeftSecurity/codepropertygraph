...
object Literal {
  ...
  val layoutInformation = new NodeLayoutInformation(
    ...
    List(edges.Dominate.layoutInformation, edges.Cdg.layoutInformation, edges.EvalType.layoutInformation, edges.Cfg.layoutInformation, edges.PostDominate.layoutInformation, edges.ReachingDef.layoutInformation, edges.TaggedBy.layoutInformation, edges.DynamicType.layoutInformation).asJava,
  ...
  object Edges {
    val Out: Array[String] = Array("CDG","CFG","DOMINATE","DYNAMIC_TYPE","EVAL_TYPE","POST_DOMINATE","REACHING_DEF","TAGGED_BY")
...
trait LiteralBase extends CpgNode with ExpressionBase with HasArgumentIndex with HasCode with HasColumnNumber with HasDepthFirstOrder with HasDynamicTypeHintFullName with HasInternalFlags with HasLineNumber with HasOrder with HasTypeFullName {
  def asStored : StoredNode = this.asInstanceOf[StoredNode]

  
}

class Literal(graph: Graph, id: Long) extends NodeRef[LiteralDb](graph, id)
  with LiteralBase
  with StoredNode
  with Expression {
...
def _arrayInitializerViaAstIn: Iterator[ArrayInitializer] = get()._arrayInitializerViaAstIn
...
class LiteralDb(ref: NodeRef[NodeDb]) extends NodeDb(ref) with StoredNode
...
def _arrayInitializerViaAstIn: Iterator[ArrayInitializer] = _astIn.asScala.collect { case node: ArrayInitializer => node }
...
override def fromNewNode(someNewNode: NewNode, mapping: NewNode => StoredNode):Unit = {
  ...
  val other = someNewNode.asInstanceOf[NewLiteral]
...
class LiteralTraversal[NodeType <: Literal](val traversal: Traversal[NodeType]) extends AnyVal {
